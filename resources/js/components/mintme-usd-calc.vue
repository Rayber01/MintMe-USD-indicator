<template>
	<div>
        ...
        <div class="row">
	        ...
	        <div v-else class="mt-2 font-weight-bold">${{formatCurrency(resultUsd)}}</div>
        </div>
    </div>
</template>
		...
		methods:{
			calculate(){
				...
				if (isNaN(this.amount) || this.amount == "") {
					...
					axios.get('/calc/mintme/vs/usd/'+this.amount)
		            .then(res => {
		                this.resultUsd = res.data;
		            })
		            .catch(err => {
		                console.error(err);
		            })
